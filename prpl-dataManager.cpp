                           ...
                           pRPL::ReadingOption readOpt)
{
    ...
    for (int iSubspc = 0; iSubspc < pvSubspcIDs->size(); iSubspc++)
    {
        ...
        { // if SubCellspace has NOT been evaluated
            if (readOpt == pRPL::PARA_READING)
            { // if parallel reading, input data has been read
            ...
            { // if centralized reading, check if transfers have completed
                const vector<string> &vInLyrNames = trans.getInLyrNames();
                for (int iInLyr = 0; iInLyr < vInLyrNames.size(); iInLyr++)
                {
             ...
             bool ifInitNoData)
{
    ...
    /* 测试 worker ST */
    // pRPL::Layer *_layer = getLayerByName(trans.getPrimeLyrName().c_str(), true);
    // cout << "layer name" << _layer->name() << endl;
    // Cellspace *_cellspace = _layer->subCellspace_glbID(0, true);
    // if (_cellspace == NULL)
    //     cout << "cellspace null" << endl;
    // int minCol = ((pRPL::SubCellspace *)_cellspace)->subInfo()->workBR().minICol();
    // int maxCol = ((pRPL::SubCellspace *)_cellspace)->subInfo()->workBR().maxICol();
    // cout << "_EVAL ALL 1" << endl;
    ...
    if (subspcGlbID == pRPL::ERROR_ID)
    { // if evaluate the whole Cellspace
        pPrmCellspace = pPrmLyr->cellspace();
    }
    else
    { // if evaluate a SubCellspace
    ...
    case pRPL::EVAL_EDGES:
        for (int iEdge = 0; iEdge < ((pRPL::SubCellspace *)pPrmCellspace)->subInfo()->nEdges(); iEdge++)
        {
            ...
            if (pWorkBR != NULL)
            {
                done = trans.evalBR(*pWorkBR, GPUCompute, pf);
              ...
              pRPL::WritingOption writeOpt)
{
    ...
    while (_nActiveWorkers > 0)
    {
        ...
        if ((pRPL::TaskSignal)req == pRPL::NEWTASK_SGNL)
        { // worker requesting new task
            if (!_mOwnerships.allMapped())
            { // there still are tasks
        ...
        else if ((pRPL::TaskSignal)req == pRPL::SUBMIT_SGNL)
        { // worker submitting result
            if (writeOpt == pRPL::PARA_WRITING ||
                writeOpt == pRPL::PARADEL_WRITING)
            { // parallel writing
                int recvSubspcID;
        ...
        else if ((pRPL::TaskSignal)req == pRPL::QUIT_SGNL)
        { // worker quitting
              ...
              pRPL::WritingOption writeOpt)
{
    ...
    while (_nActiveWorkers > 0)
    {
        ...
        if ((pRPL::TaskSignal)req == pRPL::SUBMIT_SGNL)
        { // worker submitting result
            if (writeOpt == pRPL::PARA_WRITING ||
                writeOpt == pRPL::PARADEL_WRITING)
            { // parallel writing
              ...
              const pRPL::LongVect *pvGlbIdxs)
{
    ...
    while ((pRPL::TaskSignal)resp != pRPL::QUIT_SGNL)
    { // keep running until receiving a Quitting instruction from master
        ...
        if (_mOwnerships.subspcIDsOnPrc(_prc.id())->size() - _vEvaledSubspcIDs.size() <= 1)
        {
            ...
            if ((pRPL::TaskSignal)resp != pRPL::QUIT_SGNL)
            { // received SubCellspace ID from master
                mappedSubspcID = resp;
        ...
        if (readySubspcID != pRPL::ERROR_ID)
        {
            ...
            case EVAL_NONE:
                ...
                break;
            case EVAL_ALL:
                if (_evalAll(trans, GPUCompute, pf, readySubspcID, pRPL::EVAL_WORKBR, ifInitNoData) == pRPL::EVAL_FAILED)
                {
                    _prc.abort();
                    return pRPL::EVAL_FAILED;
                }
    ...
    if (!_prc.hasWriter())
    {
        ...
        { // parallel writing
            while (_nActiveWorkers > 0)
            {
                MPI_Recv(&req, 1, MPI_INT, MPI_ANY_SOURCE, pRPL::REQUEST_TAG, _prc.comm(), &status);
                int workerID = status.MPI_SOURCE;
                if ((pRPL::TaskSignal)req == pRPL::QUIT_SGNL)
                { // worker quitting
                    // cout << "Process "<< workerID << " quit" << endl;
                    _nActiveWorkers--;
                }
              ...
              const pRPL::LongVect *pvGlbIdxs)
{
    ...
    if (pvSubspcIDs != NULL)
    {
        for (int iSubspc = 0; iSubspc < pvSubspcIDs->size(); iSubspc++)
        {
            ...
            if (!trans.needExchange() ||
                (trans.needExchange() && !trans.edgesFirst()))
            { // no exchange needed
                ...
                case EVAL_NONE:
                    ...
                    break;
                case EVAL_ALL:
                    if (_evalAll(trans, GPUCompute, pf, subspcGlbID, pRPL::EVAL_WORKBR, ifInitNoData) == pRPL::EVAL_FAILED)
                    {
                        _prc.abort();
                        return pRPL::EVAL_FAILED;
                    }
            ...
            { // exchange needed, process the edges first
                ...
                case EVAL_NONE:
                    ...
                    break;
                case EVAL_ALL:
                    if (_evalAll(trans, GPUCompute, pf, subspcGlbID, pRPL::EVAL_EDGES, ifInitNoData) == pRPL::EVAL_FAILED)
                    {
                        _prc.abort();
                        return pRPL::EVAL_FAILED;
                    }
        ...
        if (trans.needExchange())
        { // exchange, while processing the interiors
            ...
            if (trans.edgesFirst())
            {
                for (int iSubspc = 0; iSubspc < pvSubspcIDs->size(); iSubspc++)
                {
                    ...
                    case EVAL_NONE:
                        break;
                    case EVAL_ALL:
                        if (_evalAll(trans, GPUCompute, pf, subspcGlbID, pRPL::EVAL_INTERIOR, ifInitNoData) == pRPL::EVAL_FAILED)
                        {
                            _prc.abort();
                            return pRPL::EVAL_FAILED;
                        }
                   ...
                   bool pReading)
{
    ...
    if (pReading)
    { // parallel reading: all processes read the file in parallel
    ...
    { // master process reads the file and distribute info to other processes
        vector<char> vBuf;
        int bufSize;
            ...
            { // master process
                ...
                for (int iSub = 0; iSub < vMappedSubspcIDs.size(); iSub++)
                {
                    ...
                    if (mappedSubspcID >= 0 && prcID >= 0)
                    {
                        ...
                        if (prcID != _prc.id())
                        { // transfer the SubCellspace if it belongs to a worker
                ...
                const pRPL::LongVect *pvGlbIdxs)
{
    ...
    { // worker processes
        done = _workerTF(evalType, trans, readOpt, writeOpt, GPUCompute, pf, ifInitNoData, pvGlbIdxs);
    }
                ...
                const pRPL::LongVect *pvGlbIdxs)
{
    ...
    { // worker processes
        done = _workerST(evalType, trans, writeOpt, GPUCompute, pf, ifInitNoData, pvGlbIdxs);
        if (_prc.isMaster() && done != pRPL::EVAL_FAILED)
        { // master process
