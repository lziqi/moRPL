...
/* OpenCL */
void *pRPL::Cellspace::getData()
{
    return _pData;
}

void pRPL::Cellspace::updateData(void *data, size_t size)
{
...
// todo:添加容错
template <class Type>
bool pRPL::Cellspace::updateCellspace(const pRPL::CoordBR &br, void *data)
{
    bool done = false;
    Type *new_data = (Type *)data;
    Type *old_data = (Type *)_pData;
    if (_trackUpdt) //开启压缩选项
    {
        for (long row = br.minIRow(); row <= br.maxIRow(); row++)
        {
            for (long col = br.minICol(); col <= br.maxICol(); col++)
            {
                if (old_data[row * _pInfo->nCols() + col] != new_data[row * _pInfo->nCols() + col])
                {
                    _vUpdtIdxs.push_back(_pInfo->coord2idx(row, col));
                }
            }
        }
    }
...
bool pRPL::Cellspace::updateCellspaceAs(const pRPL::CoordBR &br, void *data)
{
    bool done = false;
    if (_pInfo->isDataType<bool>())
    {
        done = updateCellspace<bool>(br, data);
    }
    else if (_pInfo->isDataType<unsigned char>())
    {
        done = updateCellspace<unsigned char>(br, data);
    }
    else if (_pInfo->isDataType<char>())
    {
        done = updateCellspace<char>(br, data);
    }
    else if (_pInfo->isDataType<unsigned short>())
    {
        done = updateCellspace<unsigned short>(br, data);
    }
    else if (_pInfo->isDataType<short>())
    {
        done = updateCellspace<short>(br, data);
    }
    else if (_pInfo->isDataType<unsigned int>())
    {
        done = updateCellspace<unsigned int>(br, data);
    }
    else if (_pInfo->isDataType<int>())
    {
        done = updateCellspace<int>(br, data);
    }
    else if (_pInfo->isDataType<unsigned long>())
    {
        done = updateCellspace<unsigned long>(br, data);
    }
    else if (_pInfo->isDataType<long>())
    {
        done = updateCellspace<long>(br, data);
    }
    else if (_pInfo->isDataType<float>())
    {
        done = updateCellspace<float>(br, data);
    }
    else if (_pInfo->isDataType<double>())
    {
        done = updateCellspace<double>(br, data);
    }
    else if (_pInfo->isDataType<long double>())
    {
        done = updateCellspace<long double>(br, data);
    }
    return done;
}
